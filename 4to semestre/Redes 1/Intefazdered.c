...
unsigned char MAC[6], MASK[4], IP[4],BRD[4];  //Definimos las variables MAC, MASK (mascara de sub red) y la IP
void obtenerDatos(int ds) //Creamos una funcion para obtener los datos. ds = Descriptor de socket
{
    ...
    //Obtenemos el MTU con la funciòn ioclt (SIOCGIFMTU).
    if(ioctl(ds,SIOCGIFMTU,&nic)==-1){ //Si devuelve -1 existio un error al obtener el MTU
       perror("Error al obtener el MTU"); //avisamos que existe un error 
    }
    else{ //en caso contrario se obtuvo correctamente el MTU 
       printf("\nEl MTU es: %d\n", nic.ifr_mtu); //Imprimimos el valor del MTU 
    }
    
     //Obtenemos la dirección de broadcast (SIOCGIFBRDADDR)
    if(ioctl(ds,SIOCGIFBRDADDR,&nic)==-1){ //Si devuelve -1 existio un error al obtener la dirección de broadcast
       perror("Error al obtener la dirección de Broadcast"); //avisamos que existe un error 
    }
    else{ //en caso contrario se obtuvo correctamente
       memcpy(BRD,nic.ifr_broadaddr.sa_data, 6); //Declaramos la funcion memcpy para copiar los 4 bits en la variable BRD
       printf("\nBROADCAST: "); //Imprimimos los valores de la direcciòn de broadcast
       for(i=2; i<6;i++){
           printf("%2d:",BRD[i]);//lo imprimimos de forma decimal 
       }
    } 
